...
public final class DataSize implements Comparable<DataSize>, Serializable {
    ...
    public static DataSize parse(@NonNull CharSequence text, @Nullable DataUnit defaultUnit) {
        try {
            ...
            final String formattedStr = matcher.group(1).replace(',','.');
            final float value = Float.parseFloat(formattedStr);
            final long amount = (long)(unit.size().toBytes() * value);
            return DataSize.ofBytes(amount);
        }
        catch (Exception ex) {
    ...
    private static class DataSizeUtils {
         ...
         */
        private static final Pattern PATTERN = Pattern.compile("^(\\d*[.,]?\\d+)\\s*([a-zA-Z]{0,2})$");
